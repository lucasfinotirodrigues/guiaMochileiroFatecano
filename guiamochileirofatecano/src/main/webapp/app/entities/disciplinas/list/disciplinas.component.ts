...
export class DisciplinasComponent implements OnInit {
  ...
  trackId = (_index: number, item: IDisciplinas): number => this.disciplinasService.getDisciplinasIdentifier(item);

  ngOnInit(): void {
    this.load();
  }

  delete(disciplinas: IDisciplinas): void {
    const modalRef = this.modalService.open(DisciplinasDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.disciplinas = disciplinas;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.disciplinas = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IDisciplinas[] | null): IDisciplinas[] {
    return data ?? [];
  }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.disciplinasService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
